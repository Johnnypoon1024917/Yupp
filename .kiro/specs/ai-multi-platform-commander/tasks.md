...
- [ ] 1. Update data models and type definitions
  - [ ] 1.1 Add `ExtractedPlace` interface and `Platform` type to `src/types/index.ts`
    - Add `ExtractedPlace` with `name: string` and `contextualHints: string[]`
    - Add `Platform` type as `'instagram' | 'douyin' | 'xiaohongshu' | 'unknown'`
    - _Requirements: 1.1_
  - [ ] 1.2 Update `ScrapeResult` interface in `src/types/index.ts`
...
- [ ] 2. Implement AI extraction module `src/actions/extractPlaces.ts`
  - [ ] 2.1 Create `detectPlatform` function
  ...
  - [ ] 2.3 Create `buildExtractionPrompt` function
  ...
  - [ ] 2.5 Create `parseLLMResponse` function
  ...
  - [ ] 2.9 Implement `callDeepSeek` internal function
...
- [ ] 4. Integrate AI extraction into scraper
  - [ ] 4.1 Update `scrapeUrl` in `src/actions/scrapeUrl.ts`
...
- [ ] 5. Update MagicBar for batch geocoding and progress feedback
  - [ ] 5.1 Refactor `MagicBar` handleSubmit for multi-place flow
  ...
  - [ ] 5.2 Add batch processing status text to MagicBar
...
- [ ] 7. Fix planner DnD sensor configuration
  - [ ] 7.1 Update `usePlannerDnd` sensor setup in `src/hooks/usePlannerDnd.tsx`
...
- [ ] 8. Update environment configuration
  - [ ] 8.1 Add new environment variables to `.env.local.example`
    - Add `DEEPSEEK_API_KEY` and `GEMINI_API_KEY` placeholder entries
    - _Requirements: 10.1, 10.2_
  - [ ] 8.2 Install `@google/generative-ai` dependency
